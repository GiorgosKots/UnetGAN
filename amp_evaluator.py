...
def score_peptide(properties: Dict[str, float]) -> Dict[str, float]:
    ...
    scores = {
        'length': 1.0 if 8 <= properties['length'] <= 60 else 
...
def evaluate_amp_batch(dna_sequences: List[str], return_details: bool = False) -> Union[float, Tuple[float, Dict]]:
    """
    ...
    Returns:
        score: Overall batch score (0-100)
    ...
    if valid_sequences == 0:
        return (0.0, {'error': 'No valid sequences found'}) if return_details else 0.0
