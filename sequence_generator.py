...
OUTPUT_DIR = "D:/GANs/GANs/campr4"  # Using forward slashes is safer
...
def generate_and_filter_sequences(generator, num_samples=100000, batch_size=128, device="cuda", add_atg=False):
    """Generate sequences in batches, optionally add ATG if needed, and filter valid ones"""
    ...
    with torch.no_grad():
        for i in range(0, num_samples, batch_size):
            ...
            for seq in decoded_seqs:
                ...
                elif not add_atg and not clean_seq.startswith('ATG'):
                    continue  # Skip sequences that don't start with ATG when add_atg is False
                ...
                if ((clean_seq.endswith('TAA') or
                    ...
                    len(clean_seq) > 9):  # Ensure length is more than 9 bases
...
def convert_dna_fasta_to_protein(input_fasta=None, add_atg=False):
    ...
    with open(output_fasta, 'w') as protein_file:
        ...
        for record in SeqIO.parse(input_fasta, "fasta"):
            ...
            try:
                ...
                # Write protein sequence if it's valid (non-empty)
                if protein_seq:
                    protein_count += 1
